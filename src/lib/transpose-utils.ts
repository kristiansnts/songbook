...
export const transposeChord = (
  ...
  if (!originalScaleChords || !targetScaleChords) {
    console.warn(
      `Diatonic chords not defined for base key: ${normalizedFromKey} or target key: ${normalizedToKey}. Cannot perform diatonic transposition for chord: ${chord}.`
    )
    return chord
  }
  ...
  for (let i = 0; i < originalScaleChords.length; i++) {
    ...
    if (
      scaleChordRoot &&
      normalizeKey(scaleChordRoot) === normalizeKey(chordRoot)
    ) {
  ...
  const finalTransposedRoot = transposedRootMatch
    ? transposedRootMatch[1]
    : transposedRootWithQuality

  return finalTransposedRoot + chordSuffix
}

// Function to transpose chords in the stored styled format
export const transposeStoredChords = (
  htmlContent: string,
  fromKey: string,
  toKey: string
): string => {
