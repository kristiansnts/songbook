...
export class AuthManager {
  ...
  constructor() {
    ...
    this.baseUrl =
      import.meta.env.VITE_APP_PROD_URL_API ||
  ...
  async login(credentials: LoginCredentials): Promise<{
    ...
    try {
      ...
      const result: ApiResponse<{ user: any; token: string }> =
        await response.json()
  ...
  async hasPermission(role: 'pengurus' | 'peserta'): Promise<boolean> {
    ...
    try {
      ...
      if (response.ok) {
        const result: ApiResponse<{ hasPermission: boolean }> =
          await response.json()
      ...
      if (role === 'peserta' && this.token) {
        console.warn(
          'Allowing peserta access for development since server is not available'
        )
        return true
      }
      return false
