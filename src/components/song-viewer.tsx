...
const transposeStoredChords = (htmlContent: string, fromKey: string, toKey: string): string => {
  if (fromKey === toKey) {
...
export function SongViewer({ song }: SongViewerProps) {
  ...
  return (
    <div className="max-w-4xl mx-auto p-3 md:p-6 space-y-6">
      {/* Simple Header Section - Left Aligned */}
      <div className="space-y-2">
        <h1 className="text-2xl md:text-3xl font-bold text-gray-800">{song.title}</h1>
        <p className="text-lg md:text-xl text-gray-600">{song.artist}</p>
      </div>

      {/* Simple Key Selector Grid - Responsive */}
      <div className="grid grid-cols-6 gap-2 max-w-xs md:max-w-sm">
        {KEYS.map((key) => (
          <button
            key={key}
            onClick={() => setSelectedKey(key)}
            className={cn(
              "h-8 w-8 md:h-12 md:w-12 rounded-lg text-sm md:text-lg font-semibold transition-colors",
              selectedKey === key 
      ...
      {/* Rich Text Lyrics and Chords Section */}
      <div ref={lyricsRef}>
        {song.lyrics_and_chords && song.lyrics_and_chords.trim() !== '' ? (
          ...
          <div className="text-center text-gray-500 py-8">
            <p>No lyrics and chords available for this song.</p>
          </div>
        )}
      </div>
    </div>
