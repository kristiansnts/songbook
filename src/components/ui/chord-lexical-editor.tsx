...
function parseContentForEditing(html: string): string {
  ...
  const chordElements = tempDiv.querySelectorAll(
    '[data-chord="true"], span.c, span.on'
  )
...
function ToolbarPlugin() {
  ...
  return (
    <div className='border-border bg-muted flex flex-wrap items-center gap-1 border-b p-2 sm:gap-2'>
      <Button
        variant='ghost'
        size='sm'
        onClick={undo}
      ...
      <LexicalComposer initialConfig={initialConfig}>
        ...
        <div className='bg-background relative min-h-[200px]'>
          <PlainTextPlugin
            contentEditable={
              <ContentEditable
                className='prose prose-sm sm:prose-base text-foreground max-w-none resize-none bg-transparent p-3 text-sm leading-relaxed whitespace-pre-wrap focus:outline-none sm:p-4 sm:text-base'
                style={{ minHeight: '200px' }}
              />
            }
            placeholder={
              <div className='text-muted-foreground pointer-events-none absolute top-3 left-3 text-sm select-none sm:top-4 sm:left-4 sm:text-base'>
                {placeholder}
              </div>
            }
